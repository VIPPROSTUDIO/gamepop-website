...
// Generate a random food position that doesn't collide with the snake
function generateFoodPosition(){
    while(true){
       ...
       for(let segments of snake){
              if(newFoodPosition.x === segments.x && newFoodPosition.y === segments.y){
                collisionWithSnake = true;
                break;
            }
       }

       // Return the food position if there is no collision
...
// Check for collision with the wall of self
...
function update(){
    ...
    if(head.x === food.x && head.y === food.y){
        ...
        if(snake.length === (canvas.width / SNAKE_SIZE) * (canvas.height / SNAKE_SIZE)){
            gameWin();
            return;   
        }
    }else{
    ...
    if(blinkCounter % 4 === 0 && !gamePaused){
        food.x += food.dx;
        food.y += food.dy;
...
// Restart game wen restart button clicked
